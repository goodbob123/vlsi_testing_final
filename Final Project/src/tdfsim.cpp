...
void ATPG::tdfault_sim_a_vector(const string &vec, int &num_of_current_detect, bool &is_redundant_vector) {
  int i, nckt;
  fptr f;
  ...
  tdfault_sim_a_vector2(vec, num_of_current_detect, is_redundant_vector);

}

/* fault simulate a single test vector */
void ATPG::tdfault_sim_a_vector2(const string &vec, int &num_of_current_detect, bool &is_redundant_vector) {
  wptr w, faulty_wire;
  ...
  sorted_flist.erase(remove_if(
      sorted_flist.begin(), sorted_flist.end(),
      [&](const fptr fptr_ele) {
        if (fptr_ele->detect == TRUE) {
          return true;
        } else {
...
int ATPG::fault_rank_pattern(const string &vec) {
  int i, nckt;
  fptr f;
  ...
  return fault_rank_pattern2(vec);

}

/* fault simulate a single test vector */
int ATPG::fault_rank_pattern2(const string &vec) {
  wptr w, faulty_wire;
  /* array of 16 fptrs, which points to the 16 faults in a simulation packet  */
  fptr simulated_fault_list[num_of_pattern];
  fptr f;
  ...
  for(fptr fptr_ele : flist_undetect){
    if (fptr_ele->detected_time >= 1) {
      if(rank_method == 0) rank += fptr_ele->scoap;
      else if(rank_method == 1) rank += fptr_ele->eqv_fault_num;
      else throw std::runtime_error("rank method error!");
