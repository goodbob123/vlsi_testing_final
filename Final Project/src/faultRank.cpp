...
void ATPG::init_reach() {
    ...
    for (size_t i = 0; i < sort_wlist.size(); i++) {
        ...
        switch (n->type) {
            case AND:
                *w_rc0 |= (rc0_fi1->size() < rc0_fi2->size() ? rc0_fi1->getV1() : rc0_fi2->getV1());
                *w_rc1 |= rc1_fi1->getV1();
                *w_rc1 |= rc1_fi2->getV1();
                break;
            case BUF:
                ...
                *w_rc1 |= rc1_fi1->getV1();
                break;
            case NAND:
                *w_rc1 |= (rc0_fi1->size() < rc0_fi2->size() ? rc0_fi1->getV1() : rc0_fi2->getV1());
                *w_rc0 |= rc1_fi1->getV1();
                *w_rc0 |= rc1_fi2->getV1();
                break;
            case OR:
                *w_rc0 |= rc0_fi1->getV1();
                *w_rc0 |= rc0_fi2->getV1();
                *w_rc1 |= (rc1_fi1->size() < rc1_fi2->size() ? rc1_fi1->getV1() : rc1_fi2->getV1());
                break;
            case NOR:
                *w_rc1 |= rc0_fi1->getV1();
                *w_rc1 |= rc0_fi2->getV1();
                *w_rc0 |= (rc1_fi1->size() < rc1_fi2->size() ? rc1_fi1->getV1() : rc1_fi2->getV1());
                break;
            case NOT:
                *w_rc1 |= rc0_fi1->getV1();
                *w_rc0 |= rc1_fi1->getV1();
    ...
    for (int i = sort_wlist.size() - 1; i >= 0; i--) {
        ...
        for (nptr& n : w->onode) {
            ...
            rptr rc0_fic = w_fic->_rc0[w_fic->get_reach_id(n)];
            rptr rc1_fic = w_fic->_rc1[w_fic->get_reach_id(n)];
            ...
            switch (n->type) {
                ...
                case AND:
                    *cur_ro |= ro_fo->getV1();
                    *cur_ro |= rc1_fic->getV1();
                    break;
                case NOR:
                case OR:
                    *cur_ro |= ro_fo->getV1();
                    *cur_ro |= rc0_fic->getV1();
                    break;
                case BUF:
                case NOT:
                    *cur_ro |= ro_fo->getV1();
                ...
                case EQV:
                    *cur_ro |= ro_fo->getV1();
                    *cur_ro |= (rc0_fic->size() < rc1_fic->size() ? rc0_fic->getV1() : rc1_fic->getV1());
                    break;
                case OUTPUT: // done
